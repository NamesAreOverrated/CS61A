...
def schedule(galaxy, sum_to, max_digit):
    ...
    def schedule_helper(galaxy, sum_sofar, index):
        ...
        if index >= len(galaxy) and sum_sofar != sum_to:
            return []
        elif sum_sofar == sum_to and index >= len(galaxy):
            return [galaxy]
        elif galaxy[index].isnumeric():
            return schedule_helper(galaxy, sum_sofar+int(galaxy[index]), index+1)
        ans = []
        for x in range(max_digit+1):
            modified_galaxy = schedule_helper(
                galaxy[:index]+str(x) + galaxy[index+1:], sum_sofar+x, index+1)
            if modified_galaxy:
                ans += (modified_galaxy)
        return ans
