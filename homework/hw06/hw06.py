    ...
    def bst(t, min, max):
        ...
        if len(t.branches) == 2:
            if t.branches[0].label > t.label:
                return False
            if t.branches[1].label < t.label or t.branches[1].label > max:
                return False

            return bst(t.branches[0], min, t.label) and bst(t.branches[1], min, max)
        else:
            if t.branches[0].label > t.label:
                if t.branches[0].label > max:
                    return False
                return bst(t.branches[0], min, max)
            else:
                return bst(t.branches[0], t.label, max)
    return bst(t, 0, 99999)
...
def store_digits(n):
    ...
    while(num > 0):
        _link = Link(int(n/(10 ** num)))
        n -= ((int)((n/(10**num))) * (10**num))
        if link == None:
            link = _link
        else:
            link.rest = _link
