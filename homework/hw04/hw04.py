...
def balanced(m):
    ...
    def get_torque_by_arm(arm):
        arm_length = length(arm)
        mobile_weight = total_weight(end(arm))
        return arm_length * mobile_weight
    if get_torque_by_arm(left_arm) == get_torque_by_arm(right_arm):
...
def has_path(t, phrase):
    ...
    if len(phrase) == 0:
        return True
    return any([has_path(x, phrase) for x in branches(t)])
