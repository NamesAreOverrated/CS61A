...
def missing_digits(n):
    ...
    "*** YOUR CODE HERE ***"
    def get_last_first():
        return int(str(n)[len(str(n))-1]), int(str(n)[0])

    last, first = get_last_first()

    def missing(count=0, check_num=first+1):
        if(check_num >= last):
            return count
        if not str(n).__contains__(str(check_num)):
            count += 1
        check_num += 1
        return missing(count, check_num)
...
def count_change(total):
    ...
    def count_coin(value=total, cent=1):
        if(value == 0):
            return 1
        elif value < cent:
            return 0
        return count_coin(value, cent*2)+count_coin(value-cent, cent)
...
def make_anonymous_factorial():
    ...
    True
    """
    return lambda x: (f := lambda x: 1 if x == 0 else x*f(x - 1))(x)
