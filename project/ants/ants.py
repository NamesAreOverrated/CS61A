...
class Ant(Insect):
    ...
    @property
    def damage(self):
        if self.__is_set_dmg:
            return self.__set_dmg*2 if self.queen_dmg_buff else self.__set_dmg    
        return self._damage*2 if self.queen_dmg_buff else self._damage
    @damage.setter
    def damage(self,value):
        self.__is_set_dmg=True
        self.__set_dmg=value
    @property
    def queen_dmg_buff(self):
        return self.__queen_buff
    @queen_dmg_buff.setter
    def queen_dmg_buff(self,value):
        if value and not self.__queen_buff:
            self.__queen_buff=True
    ...
    def __init__(self, armor=1):
        ...
        self.__set_dmg=0
        self.__is_set_dmg=False
        self.__queen_buff=False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        range = 0
        while(place != None and place != beehive):
            if range == self.skip_throw:
                range += 1
                place = place.entrance
                continue
            if len(place.bees) > 0 and range >= self.min_range and range <= self.max_range:
                return place.bees[random.randrange(0, len(place.bees))]
            place = place.entrance
            range += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        place = self.place
        for bee in [b for b in place.bees]:
            bee.reduce_armor(amount)

        Ant.reduce_aror(self, amount)
        if self.armor <= 0:
            for bee in [b for b in place.bees]:
                bee.reduce_armor(
                    self.damage)
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        for bee in [b for b in self.place.bees]:
            bee.reduce_armor(
                self.damage)
        # END Problem 7
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while place != None:
            if place.ant != None:
                place.ant.queen_dmg_buff = True
                if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant != None:
                    place.ant.contained_ant.queen_dmg_buff = True
            place = place.exit
